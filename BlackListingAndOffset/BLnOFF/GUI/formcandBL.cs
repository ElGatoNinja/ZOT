...
{
    public partial class formcandBL : Form
    {
        public DataTable dt;
        public DataView dv;

        ayudaCandBL ayuda = null;
        ...
        public formcandBL(DataTable colindancias)
        {
        ...
        private void toolStripButton1_Click(object sender, EventArgs e)
        {
            ...
            ayuda = new ayudaCandBL();
            ayuda.Show();
        }

        private void btnCandidatasBL_Click(object sender, EventArgs e)
        {
                ...
                try{
                    ...
                    using (StreamWriter writer = new StreamWriter(output_path))
                    {
                        ...
                        foreach (DataRow row in data.Rows)
                        {
                            ...
                            {
                                string toCSV = "LNREL;";
                                toCSV += (string)row["Column3"] + ";";
                                toCSV += (string)row["Column3"] + ";";
                                toCSV += row["Column4"].ToString() + ";";
                                if ((string)row["Column2"] != "")
                                    toCSV += ((string)row["Column2"]).Split('-')[1] + ";";
                                else
                                    toCSV += ";";
                                toCSV += "1;";
                                toCSV += "0;";
                                toCSV += (string)row["Column5"] + ";";
                                toCSV += (string)row["Column5"];
