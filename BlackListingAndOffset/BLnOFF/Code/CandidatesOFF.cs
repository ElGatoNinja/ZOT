...
namespace ZOT.BLnOFF.Code
{
    class CandidatesOFF
    {
        ...
        private readonly string[] colNames = { "Label", "ENBID SOURCE", "LnCell SOURCE", "Name SOURCE", "ENBID TARGET", "LnCell TARGET", "Name TARGET", "Distance", "HO Success(%)", "Offset", "HO Attempts", "Blacklist", "HO errores SR", "Num colindancias", "Cols ya en BL", "Cols disp OFF", "SelectedOFF", "CandidataBL" };
            ...
            for (int i = 0; i < data.Rows.Count; i++)
            {
                if ((int)data.Rows[i]["LnCell Source"] == currentLnCell)
                {
                    if ((int)data.Rows[i]["Cols disp OFF"] > asignedOFF)
                    {
                        ...
                        if (posibleBLandOFF.Length > 0 && !(bool)posibleBLandOFF[0]["SelectedBL"])
                        {
