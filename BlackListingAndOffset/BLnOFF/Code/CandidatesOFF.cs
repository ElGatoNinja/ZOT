    ...
    class CandidatesOFF
    {
        ...
        public CandidatesOFF(TimingAdvance TA, Colindancias colin,CandidatesBL candBL)
        {
            ...
            foreach (DataRow row in colin.data.Rows)
            {
                ...
                {
                    if (TA.radioLines.Select("[LNCELL] = '" + (string)row["Name SOURCE"] + "'").Length == 0)
                        Console.WriteLine("Falta la linea " + (string)row["Name SOURCE"] + " en el Timing Advance, se salta y se continua normalmente");
                }
